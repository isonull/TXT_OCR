...
public class VocaRecognition {
	public static void naiveFeatureFetch(boolean[][] image, int[][] rects, int dataWidth, int dataHeight) {
		...
		for (int[] rect : rects) {
			...
			int width = rect[2];
			int height = rect[3];
			List<Integer> splitPos = VocaSpliter.NaiveHorizontalSplit(image, rect, 0, 255);
			...
			for (int i = 0; i < splitPos.size(); ++i) {
				...
				if (limit - base > dataWidth || height > dataHeight) {
					...
					fetchRegCharFeature(image, base, yBaseRect, limit, yBaseRect + height, dataWidth, dataHeight);
				}
			}
		}
	}

	public static boolean[] fetchRegCharFeature(boolean[][] image, int xBase, int yBase, int xLimit, int yLimit,
